...
class Renderer {
    ...
    constructor(canvas: HTMLCanvasElement, private technique: Technique, private interval: number, private connection: Connection, finished: () => void) {
    ...
    renderView(projectionMatrix: mat4) {
        ...
        if (this.mesh) {
            ...
            if (animationTime >= (ANIMATION.at(-1)?.time || 0)) {
                ...
                // const dateString = new Date().toLocaleString('en-us', { weekday:"long", year:"numeric", month:"short", day:"numeric", hour: "2-digit", minute: "2-digit", second: "2-digit"});
                const dateString = new Date().toISOString().replaceAll(':','-');
                console.log(dateString);
                fetch(`http://${SERVER_URL}/files/${this.technique.name}/${1000 / this.interval}/rendering-${dateString}.json?type=log`, {
                    method: 'POST',
