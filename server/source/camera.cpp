...
void Camera::update(GLFWwindow* window, bool focused)
{
    ...
    if (this->horizontal_angle > glm::two_pi<float>())
    {
        this->horizontal_angle -= glm::two_pi<float>();
    }

    else if (this->horizontal_angle < -glm::two_pi<float>())
    {
        this->horizontal_angle += glm::two_pi<float>();
    }
    ...
    this->up = glm::cross(this->side, this->forward);
    ...
    {
        position_change.x += delta_time * CAMERA_MOVEMENT_SPEED * glm::cos(this->horizontal_angle);
        position_change.z -= delta_time * CAMERA_MOVEMENT_SPEED * glm::sin(this->horizontal_angle);
    }
