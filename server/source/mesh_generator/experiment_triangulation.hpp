#ifndef HEADER_EXPERIMENT_TRIANGULATION
#define HEADER_EXPERIMENT_TRIANGULATION
...
#define INVALID_INTERVAL_INDEX 0xFFFFFFFF

    struct AdjacentIntervals
    {
        uint32_t left_index = INVALID_INTERVAL_INDEX;
        uint32_t right_index = INVALID_INTERVAL_INDEX;
        uint32_t middle_index = INVALID_INTERVAL_INDEX;
    };
    ...
    public:
        ...
        void process(const glm::uvec2& resolution, float triangle_scale, uint32_t min_loop_length, const glsl::Loop* loop_pointer, const glsl::LoopCount* loop_count_pointer, const glsl::LoopSegment* loop_segment_pointer, std::vector<shared::Vertex>& vertices, std::vector<shared::Index>& indices, shared::ViewMetadata& statistic, std::vector<MeshFeatureLine>& features_lines, bool export_feature_lines);
    ...
    private:
        ...
        void compute_triangulation(const glm::uvec2& resolution, float triangle_scale, const glsl::Loop* loop_pointer, const glsl::LoopSegment* loop_segment_pointer, std::vector<shared::Vertex>& vertices, std::vector<shared::Index>& indices, shared::ViewMetadata& statistic, std::vector<MeshFeatureLine>& features_lines);
